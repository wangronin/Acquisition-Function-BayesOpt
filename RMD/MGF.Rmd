...
p <- ggplot(df.MGF) + 
  ...
  scale_colour_gradientn(name = TeX('$\\mathit{t}$'), breaks = quantile(unique(df.MGF$t)), 
  ...
  labs(x = TeX('$\\mathit{x}$'), y = TeX('$\\mathit{f}$')) 

ggsave('_MGF.pdf', plot = p, width = 14, height = 9)
embed_fonts('_MGF.pdf', outfile = 'MGF.pdf')
```
...
p <- ggplot(df.GEI) + 
  ...
  scale_color_discrete(name = TeX('$\\mathit{g}$')) +
  scale_linetype_manual(name = NULL, values = c('prediction' = 'solid', 'function' = 'dashed'), guide = FALSE) + 
  scale_fill_manual(name = NULL, values = c('95% CI' = '#26989C')) + 
  labs(x = TeX('$\\mathit{x}$'), y = TeX('$\\mathit{f}$')) 
