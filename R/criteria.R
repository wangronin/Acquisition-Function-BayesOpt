...
MGF <- function(x, model, plugin = NULL, t = 1, type = "UK",
  ...
  if (sigma/sqrt(model@covariance@sd2) < 1e-06) return(0)
  
  mu.prime <- mu - t * sigma ^ 2
  
  beta <- (ymin - mu) / sigma
  beta.prime <- (ymin - mu.prime) / sigma
  
  if (!minimization) {
      mu <- -mu
  }
  
  res <- pnorm(beta.prime) * exp(ymin * t + t ^ 2 * sigma ^ 2 / 2 - mu * t) / exp(t)
  
  # for debug purposes: numerical explosion happens
  if (is.infinite(res)) browser()
...
MGF.dx <- function(x, model, plugin = NULL, t = 1, type = "UK", minimization = TRUE, 
                   envir = NULL) {
  ...
  if (s/sqrt(model@covariance@sd2) < 1e-06) {
    ...
    mu.dx <- kriging.mean.grad
    s.dx <- kriging.sd.grad
    
    if (!minimization) mu.dx <- -kriging.mean.grad
    
    term1 <- exp(ymin * t + t ^ 2 * s ^ 2 / 2 - mu * t - t)
    mu.prime.dx <- mu.dx - 2 * t * s * s.dx
    beta.prime.dx <- -(mu.prime.dx + beta.prime * s.dx) / s
    
    grad <- dnorm(beta.prime) * term1 * beta.prime.dx + 
      pnorm(beta.prime) * term1 * ((t ^ 2) * s * s.dx - t * mu.dx)
    
  }
  return(grad)
}

grad <- function(x, model, type = "UK", envir = NULL) {
  ...
  # no prediction needed if the enviroment is provided
  else {   
    toget <- matrix(c("kriging.sd", "kriging.mean", 
                      "c", "Tinv.c"), 1, 4)
  ...
  kriging.mean.grad <- t(W) %*% z + t(model@trend.coef %*% 
                                        f.deltax)
  if (type == "UK") {
    tuuinv <- solve(t(u) %*% u)
